    ...
    useEffect(() => {
        ...
        client
            ...
            : res.service
                ...
                : [];
        setService(service);
const poId = Array.isArray(res.poId)
            ? res.poId.map((elem) => ({ _id: elem._id, poId: elem.poId }))
            : res.poId
                ? [{ _id: res.poId._id, poId: res.poId.poId }]
                : [];
        setPoId(poId);
            })
            .catch((error) => {
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">PO ID</label>
                    {poId.map((elem) => (
                        <Link key={elem._id} to={`/purchaseOrder/${elem._id}`}>
                        <div>
                  {" "}
                            <p className="text-xl text-primary">{elem.poId}</p>
                            </div>
                        </Link>
                    ))}</div>
