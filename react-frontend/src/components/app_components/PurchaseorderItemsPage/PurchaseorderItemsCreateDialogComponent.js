...
const [service, setService] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [customerPoId,service], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            customerPoId: _entity?.customerPoId?._id,service: _entity?.service?._id,price: _entity?.price,quantity: _entity?.quantity,description: _entity?.description,
