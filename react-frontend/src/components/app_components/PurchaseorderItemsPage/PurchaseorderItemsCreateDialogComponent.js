...
const [service, setService] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [poId,service], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            poId: _entity?.poId?._id,service: _entity?.service?._id,price: _entity?.price,quantity: _entity?.quantity,description: _entity?.description,
    ...
    useEffect(() => {
                    ...
                    client
                        ...
                        .then((res) => {
                            setPoId(res.data.map((e) => { return { name: e['customerPoId'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.log({ error });
