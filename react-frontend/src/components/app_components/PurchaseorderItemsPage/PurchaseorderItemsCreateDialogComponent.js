...
const [poId, setPoId] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [service,poId], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            service: _entity?.service?._id,poId: _entity?.poId?._id,price: _entity?.price,quantity: _entity?.quantity,
