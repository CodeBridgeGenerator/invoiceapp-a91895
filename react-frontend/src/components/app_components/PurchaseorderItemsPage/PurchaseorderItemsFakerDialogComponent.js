...
import purchaseorderItemsFakerFactory from "./purchaseorderItemsFakerFactory";
...
const PurchaseorderItemsFakerDialogComponent = (props) => {
...
client.service("services")
    ...
    const onRun = async () => {
        let fakeData = purchaseorderItemsFakerFactory(props.user,fakerCount,customerPoIdIds,serviceIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("purchaseorderItems").create(elem));
...
export default connect(mapState, mapDispatch)(PurchaseorderItemsFakerDialogComponent);
