...
const quotationItems = Array.isArray(res.quotationItems)
            ? res.quotationItems.map((elem) => ({ _id: elem._id, service: elem.service }))
            : res.quotationItems
                ? [{ _id: res.quotationItems._id, service: res.quotationItems.service }]
                : [];
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Quotation Items</label>
                    {quotationItems.map((elem) => (
                        <Link key={elem._id} to={`/quotationItems/${elem._id}`}>
                        <div>
                  {" "}
