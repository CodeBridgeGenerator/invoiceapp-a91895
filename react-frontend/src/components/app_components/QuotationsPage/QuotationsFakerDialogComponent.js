...
import quotationsFakerFactory from "./quotationsFakerFactory";
...
client.service("quotationItems")
                        ...
                        .then((data) => {
                        setquotationItemsIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = quotationsFakerFactory(props.user,fakerCount,customerIDIds,quotationItemsIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("quotations").create(elem));
...
export default connect(mapState, mapDispatch)(QuotationsFakerDialogComponent);
