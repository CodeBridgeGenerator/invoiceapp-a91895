...
const [quotationItems, setQuotationItems] = useState([])
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            quotationID: _entity?.quotationID,customerID: _entity?.customerID?._id,quotationDate: _entity?.quotationDate,quotationItems: _entity?.quotationItems?._id,discountType: _entity?.discountType,discountValue: _entity?.discountValue,remarks: _entity?.remarks,
...
useEffect(() => {
                    // on mount quotationItems
                    client
                        .service("quotationItems")
                        .find({ query: { $limit: 10000, $sort: { createdAt: -1 }, _id : urlParams.singleQuotationItemsId } })
                        .then((res) => {
                            setQuotationItems(res.data.map((e) => { return { name: e['service'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.log({ error });
