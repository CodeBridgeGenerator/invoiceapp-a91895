...
const ReceiptsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            receiptID: _entity?.receiptID,paymentID: _entity?.paymentID?._id,dateIssued: _entity?.dateIssued,totalAmount: _entity?.totalAmount,receiptDetails: _entity?.receiptDetails,
