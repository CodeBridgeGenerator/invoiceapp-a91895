...
import receiptsFakerFactory from "./receiptsFakerFactory";
...
const ReceiptsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [paymentIDIds, setpaymentIDIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("payments")
                        .find({ query: { $select: ["_id","paymentID"] } })
                        .then((data) => {
                        setpaymentIDIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = receiptsFakerFactory(props.user,fakerCount,paymentIDIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("receipts").create(elem));
...
export default connect(mapState, mapDispatch)(ReceiptsFakerDialogComponent);
