...
import invoiceItemsFakerFactory from "./invoiceItemsFakerFactory";
...
const InvoiceItemsFakerDialogComponent = (props) => {
...
client.service("services")
    ...
    const onRun = async () => {
        let fakeData = invoiceItemsFakerFactory(props.user,fakerCount,invoiceIdIds,serviceIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("invoiceItems").create(elem));
...
export default connect(mapState, mapDispatch)(InvoiceItemsFakerDialogComponent);
