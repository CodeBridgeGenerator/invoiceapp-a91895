...
import debitNoteFakerFactory from "./debitNoteFakerFactory";
...
const DebitNoteFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = debitNoteFakerFactory(props.user,fakerCount,invoiceIDIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("debitNote").create(elem));
...
export default connect(mapState, mapDispatch)(DebitNoteFakerDialogComponent);
