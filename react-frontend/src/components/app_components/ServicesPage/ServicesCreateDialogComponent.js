...
const ServicesCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.serviceType)) {
                error["serviceType"] = `Service Type field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            serviceID: _entity?.serviceID,name: _entity?.name,serviceType: _entity?.serviceType,price: _entity?.price,description: _entity?.description,
