...
const [quotationID, setQuotationID] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [service,quotationID], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            service: _entity?.service?._id,quotationID: _entity?.quotationID?._id,price: _entity?.price,quantity: _entity?.quantity,
