...
import quotationItemsFakerFactory from "./quotationItemsFakerFactory";
...
const QuotationItemsFakerDialogComponent = (props) => {
...
client.service("invoices")
                        .find({ query: { $select: ["_id","invoiceID"] } })
                        .then((data) => {
...
client.service("services")
    ...
    const onRun = async () => {
        let fakeData = quotationItemsFakerFactory(props.user,fakerCount,quotationIDIds,invoiceIDIds,serviceIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("quotationItems").create(elem));
...
export default connect(mapState, mapDispatch)(QuotationItemsFakerDialogComponent);
