...
import paymentTermsFakerFactory from "./paymentTermsFakerFactory";
...
const PaymentTermsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = paymentTermsFakerFactory(props.user,fakerCount,invoiceIDIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("paymentTerms").create(elem));
...
export default connect(mapState, mapDispatch)(PaymentTermsFakerDialogComponent);
