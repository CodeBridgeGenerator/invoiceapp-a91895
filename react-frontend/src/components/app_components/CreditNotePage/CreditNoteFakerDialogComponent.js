...
import creditNoteFakerFactory from "./creditNoteFakerFactory";
...
const CreditNoteFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = creditNoteFakerFactory(props.user,fakerCount,invoiceIDIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("creditNote").create(elem));
...
export default connect(mapState, mapDispatch)(CreditNoteFakerDialogComponent);
