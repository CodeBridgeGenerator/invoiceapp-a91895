...
const [paymentTermsIDIds, setpaymentTermsIDIds] = useState();
const [quotationIDIds, setquotationIDIds] = useState();
const [quotationItemIds, setquotationItemIds] = useState();
const [customerPoidIds, setcustomerPoidIds] = useState();
...
client.service("paymentTerms")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setpaymentTermsIDIds(data.data);
                        });
client.service("quotations")
                        .find({ query: { $select: ["_id","quotationID"] } })
                        .then((data) => {
                        setquotationIDIds(data.data);
                        });
client.service("quotationItems")
                        .find({ query: { $select: ["_id",""] } })
                        .then((data) => {
                        setquotationItemIds(data.data);
                        });
client.service("purchaseOrder")
                        .find({ query: { $select: ["_id","customerPoId"] } })
                        .then((data) => {
                        setcustomerPoidIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = invoicesFakerFactory(props.user,fakerCount,customerIDIds,paymentTermsIDIds,quotationIDIds,quotationItemIds,customerPoidIds);
