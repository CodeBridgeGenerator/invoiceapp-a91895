...
const [invoiceItemsIds, setinvoiceItemsIds] = useState();
...
client.service("invoiceItems")
                        .find({ query: { $select: ["_id","service"] } })
                        .then((data) => {
                        setinvoiceItemsIds(data.data);
                        });
client.service("paymentTerms")
    ...
    const onRun = async () => {
        let fakeData = invoicesFakerFactory(props.user,fakerCount,customerIDIds,invoiceItemsIds,paymentTermsIDIds);
