...
const [customerPoid, setCustomerPoid] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [customerID,paymentTermsID,quotationID,customerPoid], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            invoiceID: _entity?.invoiceID,customerID: _entity?.customerID?._id,invoiceDate: _entity?.invoiceDate,dueDate: _entity?.dueDate,paymentTermsID: _entity?.paymentTermsID?._id,quotationID: _entity?.quotationID?._id,remarks: _entity?.remarks,customerPoid: _entity?.customerPoid?._id,
