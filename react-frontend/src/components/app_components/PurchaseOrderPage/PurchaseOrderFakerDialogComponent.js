...
const [purchaseOrderItemsIds, setpurchaseOrderItemsIds] = useState();
...
client.service("purchaseorderItems")
                        .find({ query: { $select: ["_id","service"] } })
                        .then((data) => {
                        setpurchaseOrderItemsIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = purchaseOrderFakerFactory(props.user,fakerCount,customerIDIds,purchaseOrderItemsIds);
...
export default connect(mapState, mapDispatch)(PurchaseOrderFakerDialogComponent);
